...
class BlokController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('pekerjaan.index', compact('blocks', 'filter', 'totalBlok'));
    }

    public function show(Block $block)
    {
        $block->load(['pemakaians', 'evaluasi']);
        return view('pekerjaan.show', compact('block'));
    }
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'kerapatan'   => 'required|in:Rendah,Sedang,Tinggi',
            ...
            'gulma_foto'  => 'nullable|image|max:5120',
            'gulma_ai_raw'=> 'nullable|string',
        ]);

        if ($request->hasFile('gulma_foto')) {
            $data['gulma_foto'] = $request->file('gulma_foto')->store('gulma', 'public');
        }
